...
const Game = () => {
    ...
    useEffect(() => {
        ...
        playerImg.src = process.env.PUBLIC_URL + '/player.png'; // 컴퓨터 환경에서.public으로 시작하는 폴더에 player.png 라는 이미지가 있다면  주소를 가져와서 보여준다
        playerImg.onload = () => {
            playerImageRef.current = playerImg; // 개발자 전용으로 이미지를 무사히 가져왔는지 확인하는 코드
            console.log('Player 이미지 가져오기 성공');
        };
        playerImg.onerror = () => { //플레이어 이미지를 가져오지 못하면 실패
        ...
        const handleKeyDown = (event) => {
            ...
            switch (event.key) {
                ...
                case 'ArrowDown': case 's' :
                    newPlayer.y = Math.min(newPlayer.y + 50, canvasRef.current.height - 40);
                    break;
    ...
    useEffect(() => {
        ...
        const context = canvas.getContext('2d'); //사진 그림 표현할 때 사용
        ...
        const updateGame = () => {
            ...
            if (playerImageRef.current) { //public에 이미지가 존재하면 이미지 보여주기
                context.drawImage(playerImageRef.current, player.x, player.y, 20, 20);
            } else {
                context.fillStyle = 'black'; //이미지가 없다면 x=40 y=40 정사각형 크기의 파란색으로 표현
    ...
    useEffect(() => {
        ...
        const interval = setInterval(addObstacle, 300); 
        return () => clearInterval(interval);
    }, [isRunning]);
    ...
    return (
        <div className="game-container">
            {/* canvas는 동영상 캡쳐나 사진을 찍고 찍은 사진을 보여줄 때 사진이 보여질 수 있는 틀을 제공
                이 코드에서는 게임 틀을 제공
            */}
            <canvas ref={canvasRef} width="900" height="500" className="game-canvas" />
