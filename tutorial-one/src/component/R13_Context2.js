...
*/
const 후손 = () => {
    const {number,setNumber} = useContext(TextContext);

    return(
        <>
            <h3>후손 컴포넌트</h3>
            <input type="number" value={number} onChange={f => {setNumber(f.target.value)}}/>
        </>
    )
}
...
// const [number,setNumber] = useContext(TextContext); [나 { 상관없음
    const {number,setNumber} = useContext(TextContext);
