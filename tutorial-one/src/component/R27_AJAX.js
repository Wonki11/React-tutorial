...
const Ajax_Fetch =() => {
    return (
        ...
        <pre>
            ...
            React에서서는 Ajax대신 fetch API 또는 Axios와 같은 라이브러리를 사용
            ...
            fetch API : 리액트에 내장되어 있는 API를 가져와서 사용
                ...
                ex) 마이페이지 api가 뭐에요? 
                    (속 뜻 : 마이페이지 주소가 뭐에요?)
                ex) 로그인 백엔드 api가 뭐에요? 
                    (속 뜻 : 로그인 데이터를 가져올 수 있는 백엔드 주소가 뭐에요?)
                ex) 게시판에서 게시물이 안보여요. API가 잘못된것 같아요.
                    (속 뜻 : 백엔드에서 데이터 가져오는 주소가 잘못돼거나 그 안에 코드가 이상하다.)
